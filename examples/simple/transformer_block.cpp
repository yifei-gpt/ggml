...
// Initialize a Q4_1 tensor with random data
...
// Initialize a Q8_0 tensor with constant value 40 (→ 2.0 after scaling)
void init_q8_0_tensor_fixed(struct ggml_tensor *a) {
    const int64_t nelements = ggml_nelements(a);
    const int blocks = nelements / QK8_0;
    printf("Initializing %d blocks (Q8_0: value=40 → 40*0.05=2.0)\n", blocks);
    for (int b = 0; b < blocks; ++b) {
...
ModelTensors init_weights(
    ...
    if (dtype == "fp32") {
        ...
        init_q8_0_tensor_fixed(m.ffn_w_gate);
        init_q8_0_tensor_fixed(m.ffn_w_up);
        init_q8_0_tensor_fixed(m.ffn_w_down);
    }
...
struct ggml_tensor *rms_norm_op(
    ...
    ggml_build_forward_expand(gf, x_normed);
...
struct ggml_tensor *apply_rope(
    ...
    ggml_build_forward_expand(gf, qk_perm);
    ...
    ggml_build_forward_expand(gf, qk_roped);
    struct ggml_tensor *qk_out = ggml_permute(ctx, qk_roped, 0, 2, 1, 3);
    ggml_build_forward_expand(gf, qk_out);
    return qk_out;
}
...
struct ggml_tensor *build_transformer_block_graph(
    ...
    ggml_silu_inplace(ctx, gate_proj);

    struct ggml_tensor *up_proj = ggml_mul_mat(ctx, m.ffn_w_up, ffn_normed);
    ggml_mul_inplace(ctx, gate_proj, up_proj);

    struct ggml_tensor *ffn_down = ggml_mul_mat(ctx, m.ffn_w_down, gate_proj);
    ggml_add_inplace(ctx, current_h, ffn_down);
...
int main(int argc, char **argv) {
    ...
    const size_t memory_pool_size = 1024LL * 1024 * 1024; // 1024 MB
    ...
    ModelTensors weights = init_weights(
        ctx_w,
        n_embd, n_head_q, n_head_kv, n_embd_head,
        n_tokens, ffn_hidden_dim,
        dtype
    );

    std::vector<double> mean_output;
    mean_output.resize((size_t)n_embd * (size_t)n_tokens, 0.0);
    ...
    for (int run_i = 0; run_i < n_runs; ++run_i) {
        ...
        int this_batch = (int)std::min<int64_t>(
            batch_size,
            (int64_t)N_INPUTS - (int64_t)inputs_counted
        );
        ...
        ggml_graph_compute_with_ctx(ctx_s, gf, /* n_threads = */ 1);

        float *out_data = (float *)output_tensor->data;
        const int64_t single_out_elems = (int64_t)n_embd * (int64_t)n_tokens;
